...
export function isMannNonVehicleVariantText(value: unknown): boolean {
  ...
  // PDF qualifier lines are not standalone engine/vehicle modifications.
  // Anchor these headings: a real description may mention them later on.
  if (/^(?:FAHRZEUGPRODUKTIONSLAND|COUNTRYOFVEHICLEMANUFACTURE|SERIESERIES63)/.test(compact)) return true;
  if (/^(?:FU?ROENUMMER|USEFOROENO|ABMOTORNR|FROMENGINENO|BISMOTORNR|UPTOENGINENO)/.test(compact)) return true;
  ...
  const variants = await prisma.$queryRaw<Array<{
    ...
    WHERE make_normalized = ${makeNormalized}
      ...
      AND regexp_replace(UPPER(COALESCE(effective_vehicle_text, vehicle_text, '')), '[^A-Z0-9]', '', 'g') !~
        '^(FAHRZEUGPRODUKTIONSLAND|COUNTRYOFVEHICLEMANUFACTURE|SERIESERIES63|FU?ROENUMMER|USEFOROENO|ABMOTORNR|FROMENGINENO|BISMOTORNR|UPTOENGINENO)'
      AND regexp_replace(UPPER(COALESCE(effective_vehicle_text, vehicle_text, '')), '[^A-Z0-9]', '', 'g') !~
