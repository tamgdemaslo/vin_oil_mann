...
type Branch = {
  id: string; name: string; address: string; phone: string | null; timezone: string; intro: string | null;
...
export default function BookingClient() {
  ...
  return <main className={styles.publicRoot}>
    ...
    <section className={styles.bookingShell}>
      {step === 1 ? <div className={styles.intro}><div><h1>Выберите сервис</h1><p>Покажем услуги, график и свободное время именно выбранного филиала.</p></div></div> : branch ? <div className={styles.branchContext}><div><small>Там где масло.</small><strong>{branch.name} · {branch.address}</strong><span>{branchHoursLabel(branch.workingHours)}</span></div><button className={styles.secondaryButton} type="button" onClick={openBranchChooser}>Изменить</button></div> : null}
      ...
      <div className={styles.workspace}><section className={styles.stage}>
        ...
        {loading ? <div className={styles.skeleton}><i /><i /><i /></div> : <div className={styles.stageBody}>
          {step === 1 && <><div className={styles.stageHeading}><MapPin aria-hidden /><div><h2>Куда вы хотите приехать?</h2><p>У каждого филиала свой график и свободные окна.</p></div></div>{branchLinkError && <div className={styles.warning} role="alert">{branchLinkError}</div>}<div className={styles.choiceList}>{branches.map((item) => <label key={item.id} className={item.id === branchId ? styles.selectedChoice : ""}><input type="radio" name="branch" checked={item.id === branchId} onChange={() => chooseBranch(item.id)} /><span><strong>{item.name}</strong><small>{item.address}</small><small>{branchHoursLabel(item.workingHours)}</small></span><Check aria-hidden /></label>)}{!branches.length && <div className={styles.empty}>Онлайн-запись пока не открыта ни в одном филиале с указанным адресом.</div>}</div></>}
