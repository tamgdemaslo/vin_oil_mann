...
export async function GET(request: NextRequest) {
  ...
  const local = await searchLocalProducts({ search, oem, mannName, params: paramsValue, limit: limit + offset });
  const rows = local.products.slice(offset, offset + limit).map((row) => ({
    id: row.id,
    name: row.name,
    meta: row.meta,
    price: row.price,
    ...
    reserve: row.reserveQuantity ?? 0,
    quantity: row.availableQuantity ?? row.stockQuantity ?? 0,
  }));

  return NextResponse.json({ rows, total: local.products.length });
}
