...
type LinkAppointmentRequest = LinkDemandToAppointmentBody & {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!shipmentId) return NextResponse.json({ error: "id отгрузки не указан" }, { status: 400 });

  let body: LinkAppointmentRequest;
  try {
  ...
  if (!appointmentId) return NextResponse.json({ error: "Укажите запись для связи" }, { status: 400 });
