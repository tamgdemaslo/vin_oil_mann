...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await lookupVehicle({ organizationId, inputType: "vin", input: parsed.data.vin, extended: true, refresh: parsed.data.refresh, actorLogin: access.session.user.login });
