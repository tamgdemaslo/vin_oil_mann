...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success || !parsed.data.frame) return NextResponse.json({ error: "Укажите номер кузова" }, { status: 400 });
  ...
  try {
    const result = await lookupVehicle({ organizationId, inputType: "frame", input: parsed.data.frame, refresh: parsed.data.refresh, actorLogin: access.session.user.login });
