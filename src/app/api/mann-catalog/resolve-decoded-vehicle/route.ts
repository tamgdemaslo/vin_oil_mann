...
const bodySchema = z.object({
  ...
  shipmentId: z.string().trim().min(1).max(128).optional(),
  decodedVehicleId: z.string().trim().min(1).max(128).optional(),
  normalizedVehicle: z.custom<NormalizedVehicleIdentity>((value) => Boolean(value) && typeof value === "object"),
});
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await resolveMannVehicle({
      organizationId: parsed.data.organizationId ?? "default",
      warehouseId: parsed.data.warehouseId,
      vehicle: parsed.data.normalizedVehicle,
    });
