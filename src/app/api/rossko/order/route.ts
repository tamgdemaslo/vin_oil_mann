...
import { RosskoCheckoutPart, rosskoCheckout, rosskoConfig } from "@/lib/rossko";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const deliveryId = (cfg.deliveryId || body.delivery_id || "").trim();
    const addressId = (cfg.addressId || body.address_id || "").trim();
    const paymentId = (cfg.paymentId || body.payment_id || "").trim();
    const requisiteId = (cfg.requisiteId || body.requisite_id || "").trim();
