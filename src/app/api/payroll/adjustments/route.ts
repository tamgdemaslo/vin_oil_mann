...
export async function GET(request: NextRequest) {
  ...
  return runWithBranchApiContext(access.context, async () => {
    ...
    try {
      ...
      console.error("Payroll adjustments list failed", error);
      return NextResponse.json({ error: "Не удалось загрузить корректировки" }, { status: 500 });
    }
  });
}
...
export async function POST(request: NextRequest) {
  ...
  return runWithBranchApiContext(access.context, async () => {
    ...
    if (!body || typeof body !== "object") {
      return NextResponse.json({ error: "Некорректные данные корректировки" }, { status: 400 });
    }
    ...
    try {
      ...
      console.error("Payroll adjustment create failed", error);
