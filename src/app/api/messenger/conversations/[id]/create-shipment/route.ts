...
import { createShipmentForConversation } from "@/lib/messenger/messenger-context";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await createShipmentForConversation(id, await readJson(request), access.actor), { status: 201 });
  } catch (error) {
