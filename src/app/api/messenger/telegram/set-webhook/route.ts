...
import { setWebhook } from "@/lib/messenger/channels/telegram";
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as { dropPendingUpdates?: unknown };
  return NextResponse.json(await setWebhook({ dropPendingUpdates: body.dropPendingUpdates === true }));
}
