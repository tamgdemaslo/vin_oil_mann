...
const EDITABLE_ATTR_NAMES = ["vin номер", "модель авто", "год", "гос. номер", "пробег"];
...
function buildMaintenanceMessage(result: VinLookupResult, offers: MaintenanceOffer[]): string {
  ...
  const body = offers.flatMap((offer, index) => [
    ...
    ...offer.lines.map((line) => {
      ...
      return `${line.name}${article} — ${line.quantity} шт. x ${formatMoney(line.unitPrice, line.currency)} = ${formatMoney(line.total, line.currency)}`;
    }),
...
export default function NewShipmentPage() {
  ...
  const [attributes, setAttributes] = useState<{ id: string; name: string; type: string; meta: Meta; value: string | null }[]>([]);
  ...
  const editableAttributes = attributes
    .map((a, index) => ({ a, index }))
    .filter(({ a }) => EDITABLE_ATTR_NAMES.includes((a.name ?? "").toString().toLowerCase()));
                                ...
                                >
                                  <div className="mb-2 flex items-center justify-between gap-2">
                                    <div>
                                      ...
                                      <p className="text-xs text-zinc-500 dark:text-zinc-400">
                                        ...
                                        {section.items.some((i) => i.quantity > 0)
                                          ? ` · в наличии: ${section.items.filter((i) => i.quantity > 0).length}`
                                          : ""}
                                      </p>
                                    </div>
                                    <button
                                      type="button"
                                      disabled={section.items.every((i) => i.quantity <= 0)}
                                      onClick={() => addFromVinLookup(section.items.filter((i) => i.quantity > 0))}
                                  ...
                                  <div className="space-y-2">
                                    {section.items.map((item, idx) => (
                                      <div
                                        key={item.productId ?? `${section.kind}-${idx}`}
                  ...
                  <tr key={p.assortmentMeta?.href ?? index} className="border-b border-zinc-100 dark:border-zinc-700">
                    ...
                    <td className="px-2 py-2 text-right">
                      <input
                        ...
                        onChange={(e) => {
                          const next = [...positions];
                          next[index] = { ...p, quantity: Number(e.target.value) || 0 };
                          setPositions(next);
                        }}
                        className="w-20 rounded border border-zinc-300 bg-white px-2 py-1 text-right text-xs dark:border-zinc-600 dark:bg-zinc-900"
                      />
