...
- **БД:** PostgreSQL. В `.env.local` укажите `DATABASE_URL=postgresql://...`. После этого выполните:
  ...
  npx prisma db push
  ```
- **Кассовая смена:** единый статус рабочего дня филиала. Администратор или владелец открывает её в разделе «Касса»; отдельных личных рабочих смен нет.
- **Зарплата:** владелец назначает смены для расчёта в календаре раздела «Зарплата». Эти назначения не являются операционным статусом сотрудника.
