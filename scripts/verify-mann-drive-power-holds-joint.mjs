...
const planRaw=await readFile(resolve(dir,'plan.json'),'utf8'),plan=JSON.parse(planRaw),repairRaw=await readFile(resolve(dir,'drive-power-conflict-holds-v1.json'),'utf8'),repair=JSON.parse(repairRaw);assert.equal(sha(planRaw),repair.afterPlanHash);
