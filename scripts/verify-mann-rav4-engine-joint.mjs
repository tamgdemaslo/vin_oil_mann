...
const draftRaw=await readFile(resolve(dir,'rav4-engine-scope-repair-drafts-v1.json'),'utf8'),batch=JSON.parse(draftRaw);
assert.equal(sha(raw),'f0ea2b6ece3d08ce654b5d3e3b8d536ca0b419e7f996ee82f2997421f7ac4339');
assert.equal(batch.planHash,plan.mercedesGainedScopedHistory.parentPlanHash);
...
for(const d of batch.drafts){
 ...
 delete restored.provenanceJson.engineIntersectionRepair;assert.deepEqual(restored,d.before);
}
...
for(const d of batch.drafts){
 ...
 for(let m=month(win.from)-1;m<=month(win.to)+1;m++)for(const engineCode of [...new Set([...d.before.applicabilityJson.matchedEngineScope,'UNRELATED_TEST_ENGINE'])]){
...
for(const {variant,context} of contexts.values())for(const type of [undefined,'automatic','manual','cvt','robot']){
 ...
 const retained=profile(before.filter(r=>!replacements.has(r.id)),type,context);
 for(const item of retained.items)assert.ok(actual.has(fingerprint(item)),'Unrelated fluid payload lost');
