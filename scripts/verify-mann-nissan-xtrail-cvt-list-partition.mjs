...
const [raw,sql,archive]=await Promise.all([readFile(resolve(dir,'nissan-xtrail-cvt-list-preview-v1.json'),'utf8'),readFile('/tmp/vehicle_fluid_requirements.sql','utf8'),readFile(resolve(root,'../vin-oil-mann/outputs/podbormasla-20260723/podbormasla_rows.ndjson'),'utf8')]);
...
for(const revision of s.revisions){
 ...
 const lines=anchor.application.split('\n').filter(line=>line.startsWith('- '));assert.ok(lines.length===1||lines.length===2);
 const engines=lines.map(line=>{
  ...
  if(lines.length===1){assert.equal(line,'- QR25DE / 171 л.с.');const years=/^([0-9]{4}) *- *([0-9]{4})$/.exec(anchor.production_years);assert.ok(years);return {engineCode:'QR25DE',powerHp:171,hybrid:false,yearFrom:+years[1],yearTo:+years[2],sourcePhrase:line};}
 ...
 const metadata=label(original.systemNameRaw,original.componentModel);assert.equal(metadata.transmissionType,'cvt');assert.equal(metadata.transmissionGearCount,null);
 // Independent full source-list check, retaining the manufacturer name.
 assert.equal(original.componentModel,'- RE0F10D - Jatco JF016E');const models=['RE0F10D','JATCO JF016E'];
 const required={type:'cvt',models},types=models.map(model=>JSON.stringify({type:'cvt',model}));
 assert.deepEqual(revision.provenanceJson.explicitTransmissionModelList,{policy:'EXPLICIT_CVT_SOURCE_MODEL_LIST_V1',sourceComponentModel:original.componentModel,models});
 const domain=new Set(),assigned=new Set();
 for(const e of engines)for(const type of types)for(let n=e.yearFrom*12;n<(e.yearTo+1)*12;n++)domain.add(key(e,type,n));
 assert.deepEqual(revision.applicabilityJson.matchedEngineScope,[selected.engineCode]);assert.equal(revision.applicabilityJson.transmissionType,required.type);assert.ok(!('transmissionGearCount' in revision.applicabilityJson));assert.equal(revision.applicabilityJson.componentModel,original.componentModel);
 const w=revision.applicabilityJson.window.intersection;for(const type of types)for(let n=index(w.from);n<=index(w.to);n++){const k=key(selected,type,n);assert.ok(domain.has(k));assert.ok(!assigned.has(k));assigned.add(k);covered++;}
 for(const p of s.pending.filter(p=>p.sourceRequirementId===original.id)){
  ...
  assert.deepEqual(p.requiredTransmission,required);for(const type of types)for(let n=index(p.window.from);n<=index(p.window.to);n++){const k=key(p.sourceEngineBranch,type,n);assert.ok(domain.has(k));assert.ok(!assigned.has(k));assigned.add(k);pending++;}
 }
 assert.deepEqual([...assigned].sort(),[...domain].sort());total+=domain.size;
}
assert.equal(total,672);assert.equal(covered,410);assert.equal(pending,262);assert.equal(s.pending.length,3);
...
await writeFile(resolve(dir,'nissan-xtrail-cvt-list-partition-v1.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(report));
