...
const lists=false,considered=1;
const root=resolve(import.meta.dirname,'..'),dir=resolve(root,'outputs/mann-nissan-xtrail-cvt-preview-2026-09-14');
const [auditRaw,sourceRaw,mannRaw,planRaw]=await Promise.all([readFile(resolve(dir,'xtrail-table-recheck-v1.json'),'utf8'),readFile('/tmp/vehicle_fluid_requirements.sql','utf8'),readFile('/tmp/mann_filter_applications.sql','utf8'),readFile(resolve(dir,'plan.json'),'utf8')]);
...
const selected=audit.findings.filter(f=>f.sourceRequirementId==='ca289d8522c1ca03a90a553b758237d1dc44cfcab4df901a4c07fe866976d08d');assert.equal(selected.length,1);
...
for(const finding of selected){
 ...
 assert.equal(finding.engine.engineCode,'MR20DD');assert.equal(finding.engine.powerHp,144);assert.equal(finding.engine.hybrid,false);
 ...
 const entry={...finding,metadata:finding.sourceLabelContext,vehicleVariantKey:candidate.variantIds[0],matchedEngineScope:[finding.engine.engineCode],sourceEngineContext:finding.engine,sourceEvidence:{rowId:audit.anchor.row_id,rowHash:sha(audit.anchor),originalRow:audit.anchor},candidate};
 ...
 const original=overlay.originalById.get(entry.sourceRequirementId),source=sources.get(original.id);assert.equal(sha(original),entry.sourceHash);assert.notEqual(source.rawRequirementJson?.sourceIdentity?.reviewRequired,true);assert.equal(source.transmissionType,'manual');assert.equal(entry.metadata.transmissionType,source.transmissionType);assert.equal(entry.metadata.transmissionGearCount,6);
 ...
 for(let n=entry.sourceEngineContext.yearFrom*12-1;n<=(entry.sourceEngineContext.yearTo+1)*12;n++)for(const engineCode of [...entry.matchedEngineScope,'MR20-RM31','WRONG',undefined])for(const selectedType of [undefined,'automatic','manual','cvt','robot'])for(const transmissionModel of [...correctModels,'722.000',undefined,...(lists?[source.componentModel]:[])])for(const transmissionGearCount of [count,count+1,undefined]){
   ...
   assert.equal(items.length,expected?1:0);if(expected){assert.equal(items[0].capacities.length,1);assert.equal(items[0].capacities[0].nominalLiters,2);assert.equal(items[0].automaticSelectionEligible,false);}checks++;
 }
 revisions.push(revision);
 const month=n=>`${Math.floor(n/12)}-${String(n%12+1).padStart(2,'0')}`;
 for(const e of audit.sourceEngineBranches){
  let start=null;
  for(let n=e.yearFrom*12;n<=(e.yearTo+1)*12;n++){
   ...
   if((end||visible)&&start!==null){pending.push({sourceRequirementId:source.id,sourceHash:sha(original),originalSource:original,sourceEngineBranch:e,window:{from:month(start),to:month(n-1)},requiredTransmission:{type,model:correctModels[0],gearCount:count},reason:e.hybrid?'HYBRID_ENGINE_NOT_MATCHED':'UNCOVERED_ENGINE_TRANSMISSION_MONTHS',publicationAllowed:false});start=null;}
  }
 }

}
assert.equal(revisions.length+review.length,considered);assert.equal(new Set(revisions.map(r=>r.id)).size,revisions.length);
const files=['scripts/build-mann-nissan-xtrail-manual-preview.mjs','scripts/lib/mann-nissan-xtrail-engine-branches.mjs','scripts/lib/mann-gearbox-source-quality.mjs','src/lib/mann-technical-applicability.ts','src/lib/mann-unified-technical-profile.ts','src/lib/fluid-capacity-parser.ts'];
const report={planHash:sha(planRaw),auditHash:sha(auditRaw),sourceHash:sha(sourceRaw),mannHash:sha(mannRaw),liveHash:sha(liveRaw),rawHash:audit.rawHash,sourceEngineBranches:audit.sourceEngineBranches,
...
await writeFile(resolve(dir,'nissan-xtrail-manual-preview-v1.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(report.summary));
