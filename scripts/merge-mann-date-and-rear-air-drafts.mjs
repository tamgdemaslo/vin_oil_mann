...
const path=resolve(dir,'plan.json'),reportPath=resolve(dir,'date-and-rear-air-merge-v1.json'),mode=process.argv[2];
const raw=await readFile(path,'utf8');
if(mode==='prepare'){
 const plan=JSON.parse(raw), inputs=[];
 for(const name of ['clear-date-fluid-drafts-v1.json','pajero-rear-air-capacity-drafts-v1.json']){
 ...
 assert.deepEqual(inputs.map(i=>i.data.newRevisions.length),[6,5]);
 assert.deepEqual(inputs.map(i=>i.data.summary.profileChecks??i.data.summary.checks),[4536,13122]);
 const draft={newRevisions:inputs.flatMap(i=>i.data.newRevisions)},draftRaw=JSON.stringify(inputs.map(({name,hash})=>({name,hash})));
 assert.equal(plan.newRevisions.length,2007);assert.equal(new Set(draft.newRevisions.map(r=>r.id)).size,11);
 ...
 for(const r of draft.newRevisions){
  ...
  const policy=r.provenanceJson.catalogPreviewPolicy; const fingerprint=policy==='MANN_CONDITIONAL_CAPACITY_PREVIEW_V1'?sha({policy,sourceRequirementId:r.sourceRequirementId,vehicleVariantKey:r.vehicleVariantKey,applicabilityJson:r.applicabilityJson,technicalDataJson:r.technicalDataJson}):sha({policy,sourceRequirementId:r.sourceRequirementId,vehicleVariantKey:r.vehicleVariantKey,applicability:r.applicabilityJson,technical:r.technicalDataJson});
 ...
 const summary={...plan.summary,candidateRevisions:all.length,sourceRequirements:new Set(all.map(r=>r.sourceRequirementId)).size,dateAndRearAirRevisions:additions.length};
 ...
 const updated=JSON.parse(next);assert.equal(updated.newRevisions.length,2018);assert.equal(new Set(updated.newRevisions.map(r=>r.id)).size,2018);
 assert.deepEqual(updated.newRevisions.slice(11),plan.newRevisions);assert.deepEqual(updated.existingActions,plan.existingActions);assert.deepEqual(updated.protectedBranches,plan.protectedBranches);
 await writeFile(reportPath,JSON.stringify({parentPlanHash:sha(raw),afterPlanHash:sha(next),groups,summary,added:11,preserved:2007,actionsChanged:0,publicationAllowed:false},null,2)+'\n',{flag:'wx'});
 ...
 if(mode==='patch'){
  ...
  assert.equal(sha(restored),report.parentPlanHash);console.log(JSON.stringify({wholeFileReversalVerified:true,planHash:sha(raw),added:11,preserved:2007,actionsChanged:0}));
