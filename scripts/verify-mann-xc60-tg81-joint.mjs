...
const draftRaw=await readFile(resolve(dir,'xc60-tg81-conditional-drafts-v1.json'),'utf8'),batch=JSON.parse(draftRaw);
assert.equal(sha(raw),batch.planHash);
assert.equal(batch.revisions.length,4);
...
for(const {variant,context:baseContext} of contexts.values())for(const type of [undefined,'automatic','manual','cvt','robot'])for(const transmissionModel of [undefined,'TG-81SC','TF-80SC'])for(const transmissionGearCount of [undefined,8,6]){
 const context={...baseContext,transmissionModel,transmissionGearCount};
...
const sourceRaw=await readFile('/tmp/vehicle_fluid_requirements.sql','utf8'),sources=new Map(parseCopy(sourceRaw,'vehicle_fluid_requirements').map(r=>[r.id,r]));
const preRaw=await readFile(resolve(dir,'xc60-source-branch-preflight-v3.json'),'utf8'),pre=JSON.parse(preRaw);assert.equal(sha(preRaw),batch.preflightHash);assert.equal(sha(sourceRaw),pre.sourceHash);
const {parseSpecifications}=await j.import('../src/lib/fluid-catalog.ts');
let identityNegativeCases=0;
for(const draft of batch.revisions){
 const source=sources.get(draft.sourceRequirementId),data=draft.technicalDataJson;
 for(const key of ['fillVolumeText','specificationText','recommendationText','replacementIntervalText','replacementKmMin','replacementKmMax','replacementMonths','controlIntervalText','analogText'])assert.deepEqual(data[key],source[key]);
 assert.deepEqual(data.specifications,parseSpecifications(source.specificationText,source.viscosityGradesJson??[]));assert.deepEqual(data.viscosityGrades,source.viscosityGradesJson);
 const f=pre.findings.find(f=>f.sourceRequirementId===source.id);assert.equal(f.sections.status,'NO_EXPLICIT_MARKER');assert.deepEqual(f.cautions,[]);
 const scope=draft.applicabilityJson,positive={...scope.sourceVehicleScope,engineCode:scope.matchedEngineScope[0],productionMonth:scope.window.intersection.from,transmissionModel:'TG-81SC',transmissionGearCount:8};
 const tests=[{...positive,engineCode:undefined},{...positive,engineCode:'WRONG'},{...positive,make:'TOYOTA'},{...positive,model:'XC90'},{...positive,generation:'II'},{...positive,productionMonth:undefined},{...positive,productionMonth:'INVALID'}];
 for(const context of tests)for(const type of [undefined,'automatic','manual','cvt','robot']){assert.equal(profile([fixture(draft)],type,context).items.length,0);identityNegativeCases++;}
 const planned=fixture(draft);planned.run.status='PLANNED';assert.equal(profile([planned],'automatic',positive).items.length,0);identityNegativeCases++;
}
assert.equal(sha(await readFile(resolve(dir,'plan.json'),'utf8')),sha(raw));
const report={kind:'XC60_TG81_JOINT_VERIFICATION',planHash:sha(raw),draftHash:sha(draftRaw),summary:{cases,identityNegativeCases,technicalSourcesPreserved:batch.revisions.length,variants:new Set(batch.revisions.map(d=>d.vehicleVariantKey)).size,newRevisionsSeen:batch.revisions.filter(d=>seen.has(d.id)).length,newConflicts:conflicts.size},conflicts:[...conflicts.values()],unseenDrafts:batch.revisions.filter(d=>!seen.has(d.id)).map(d=>d.id),productionApplyAllowed:false,limitations:['Local synthetic completed staging runs; no deployed VIN or database writes.','Every affected month/engine with five transmission types and missing/correct/wrong model and gear-count combinations; unrelated equipment confirmations not enumerated.','Canonical plan and downstream reports unchanged.']};
await writeFile(resolve(dir,'xc60-tg81-joint-v1.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});
