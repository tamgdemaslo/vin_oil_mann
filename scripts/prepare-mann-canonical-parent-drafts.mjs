...
assert.equal(drafts.length,priority?219:drive?218:rearAir?217:latest?217:current?213:203);assert.equal(existing.length,priority?326:drive?325:rearAir?325:expanded?324:312);
const summary={required:priority?545:drive?543:rearAir?542:expanded?541:latest?529:current?525:515,newParentDrafts:drafts.length,existingPreserved:existing.length,existingLiteralDifferences:existing.filter(r=>r.differences.length).length,differenceFields:Object.fromEntries([...Map.groupBy(existing.flatMap(r=>r.differences),d=>d.field)].map(([k,v])=>[k,v.length]))};
