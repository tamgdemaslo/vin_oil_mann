...
ROLLBACK;`;
const result=spawnSync('/opt/homebrew/bin/psql',['-X','-q','-A','-t','-v','ON_ERROR_STOP=1','-c',sql],{env,encoding:'utf8',timeout:90000,maxBuffer:160*1024*1024});
if(result.status!==0){
...
const live=new Map(),revisions=[];let transaction;
