...
const drafts=[],policy='MANN_CONDITIONAL_CAPACITY_PREVIEW_V1';let checks=0;
for(const f of pre.findings.filter(f=>['e32ac664642b80720b6fd53cb5a1826f1b013c95cfc12a437878f52f8d16dbda','29600d54bf3f848d94e390e5fe84b461c75ad0e9dce87a2b8207308cc84969fa'].includes(f.sourceRequirementId))){
 assert.deepEqual(f.reasons,['CAPACITY_OR_SERVICE_CONDITION']);const s=sources.get(f.sourceRequirementId);assert.equal(sha(s),f.sourceHash);assert.equal(plan.newRevisions.some(r=>r.sourceRequirementId===s.id),false);
 ...
 f.scope={...f.scope,engineCodes:s.engineCodesJson};
 const parsed=parseConditional(s.fillVolumeText,s.systemCode,s.engineCodesJson);assert.equal(parsed.status,'structured');
 const selected=parsed.branches.filter(b=>b.condition.kind==='engine'&&b.condition.value===f.scope.matchedEngineScope[0]);assert.equal(selected.length,1);
 const technical={fillVolumeText:s.fillVolumeText,capacityBranches:selected.map(b=>({condition:b.condition,sourceSegment:b.sourceSegment,applicabilityJson:f.scope,validation:f.target})),specificationText:s.specificationText,specifications:s.specificationsJson,viscosityGrades:s.viscosityGradesJson,recommendationText:s.recommendationText,replacementIntervalText:s.replacementIntervalText,replacementKmMin:s.replacementKmMin,replacementKmMax:s.replacementKmMax,replacementMonths:s.replacementMonths,controlIntervalText:s.controlIntervalText,analogText:s.analogText};
 ...
 for(let m=from-1;m<=to+1;m++)for(const engineCode of [f.scope.matchedEngineScope[0],'WRONG',undefined])for(const confirmedMarket of [f.scope.requiredMarket,'UNKNOWN',undefined]){
  ...
  if(expected){const item=after.items.find(i=>i.revisionId===r.id);assert.ok(item);assert.deepEqual(item.capacities,isolated.items[0].capacities);assert.deepEqual(item.capacities.map(c=>c.nominalLiters),[selected[0].capacity.nominalLiters]);assert.deepEqual(item.specifications,isolated.items[0].specifications);
