...
const [raw,sRaw,pRaw,sql]=await Promise.all([readFile(resolve(dir,'plan.json'),'utf8'),readFile(resolve(dir,'nissan-xtrail-cvt-list-preview-v1.json'),'utf8'),readFile(resolve(dir,'nissan-xtrail-cvt-list-partition-v1.json'),'utf8'),readFile('/tmp/vehicle_fluid_requirements.sql','utf8')]);
...
assert.deepEqual(s.summary,{considered:2,revisions:2,review:0,checks:58560});
assert.equal(p.total,672);assert.equal(p.covered,410);assert.equal(p.pending,262);assert.equal(p.noOverlaps,true);assert.equal(p.noGaps,true);assert.equal(s.pending.length,3);assert.equal(s.revisions.length,2);
...
for(const r of s.revisions){
 ...
 const original=sources.get(r.sourceRequirementId);assert.ok(original);assert.equal(original.systemCode,'CVT_TRANSMISSION');assert.equal(r.systemCode,original.systemCode);assert.equal(r.componentModel,original.componentModel);
 ...
 const capacity=parseFluidCapacities(original.fillVolumeText,original.systemCode);assert.equal(capacity.needsReview,false);assert.equal(capacity.capacities.length,1);assert.equal(capacity.capacities[0].nominalLiters,7.5);assert.deepEqual(r.technicalDataJson.capacities,capacity.capacities);
 assert.ok(!denied.has(r.provenanceJson.sourceAssociationFingerprint));
 assert.equal(r.applicabilityJson.transmissionType,'cvt');assert.equal(r.applicabilityJson.componentModel,original.componentModel);assert.ok(!('transmissionGearCount' in r.applicabilityJson));
 assert.deepEqual(r.provenanceJson.explicitTransmissionModelList,{policy:'EXPLICIT_CVT_SOURCE_MODEL_LIST_V1',sourceComponentModel:original.componentModel,models:['RE0F10D','JATCO JF016E']});
 assert.equal(original.componentModel,'- RE0F10D - Jatco JF016E');
 assert.deepEqual(r.applicabilityJson.matchedEngineScope,[r.provenanceJson.sourceEngineScope.engineCode]);
 const anchor=r.provenanceJson.sourceEngineEvidence.originalRow;let engines=nissanXtrailEngineBranches(anchor.model);
 if(!engines){assert.equal(anchor.model,'- QR25DE / 171 л.с.');assert.equal(anchor.production_years,'2013 - 2022');engines=[{engineCode:'QR25DE',powerHp:171,hybrid:false,yearFrom:2013,yearTo:2022,sourcePhrase:anchor.model}];}
 assert.deepEqual(engines,r.provenanceJson.sourceEngineBranches);
 for(const entry of s.pending.filter(e=>e.sourceRequirementId===r.sourceRequirementId)){assert.equal(entry.sourceRequirementId,r.sourceRequirementId);assert.deepEqual(entry.originalSource,original);assert.equal(entry.sourceHash,sha(original));assert.ok(engines.some(e=>sha(e)===sha(entry.sourceEngineBranch)));assert.equal(entry.publicationAllowed,false);assert.deepEqual(entry.requiredTransmission,{type:'cvt',models:['RE0F10D','JATCO JF016E']});if(entry.sourceEngineBranch.hybrid)assert.equal(entry.sourceEngineBranch.powerHp,null);}
...
const plan={...parent,newRevisions,nissanTransmissionPending:[...(parent.nissanTransmissionPending??[]),...s.pending],nissanTransmissionHistory:{previous:parent.nissanTransmissionHistory??null,parentHash:sha(raw),parentPath:resolve(dir,'plan.json'),supplementHash:sha(sRaw),partitionHash:sha(pRaw),addedIds:s.revisions.map(r=>r.id)},summary:{...parent.summary,candidateRevisions:newRevisions.length,sourceRequirements:new Set(newRevisions.map(r=>r.sourceRequirementId)).size,nissanTransmissionAdded:(parent.summary.nissanTransmissionAdded??0)+2},productionApplyAllowed:false};
...
const out=resolve(root,'outputs/mann-nissan-xtrail-cvt-list-preview-2026-09-14');await mkdir(out);const serialized=JSON.stringify(plan,null,2)+'\n';await writeFile(resolve(out,'plan.json'),serialized,{flag:'wx'});
const proof={planHash:sha(serialized),parentHash:sha(raw),added:2,preserved:parent.newRevisions.length,changedActions:0,newPendingIntervals:3,productionApplyAllowed:false};await writeFile(resolve(out,'merge-verification.json'),JSON.stringify(proof,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify({...proof,revisions:newRevisions.length,sources:plan.summary.sourceRequirements,out}));
