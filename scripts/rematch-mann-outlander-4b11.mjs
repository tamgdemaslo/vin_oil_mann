...
const classificationRaw=await readFile(resolve(dir,'candidate-level-source-gap-classification-v1.json'),'utf8'),classification=JSON.parse(classificationRaw),planRaw=await readFile(resolve(dir,'plan.json'),'utf8');assert.equal(sha(planRaw),'813977a2654f21eac4510323cd8491eb8be59a5bf8b25c38e15aaea9fad4c444');
...
const findings=[];
const wanted=new Set(['e08e352c77c31bdd09c4a64226a9f0fe8f29a2be238196f4a106b8ec84ff5ea5','ec0cc9c845d4964c4706eefc5b3db60039810071bed6eca797f2eedccebb3d8d','ab6a89ae320922740c048ec5b722d19fdc6524599d3f88c51c62be2fa70a56fb']);
for(const link of classification.prioritizedSourceTargets.filter(l=>wanted.has(l.sourceRequirementId)&&l.vehicleVariantKey==='2e168e7490ea1db60138f2d8b031ce57ad4fc7ee4ce9094f0a49a465995c6a23')){
