...
for(const [variant,group] of variants){
  ...
  for(const context of contexts.values())for(const type of [undefined,'automatic','manual','cvt','robot']){
    ...
    for(const [system,items] of Map.groupBy(actual.items,i=>i.systemCode)){
      if(['v4','v5','v7','v8'].includes(process.argv[3]))for(let i=0;i<items.length;i++)for(let j=i+1;j<items.length;j++){
...
const report={kind:'COMBINED_PROFILE_COMPOSITION_AUDIT',planSha256:sha(raw),variants:variants.size,cases,multiSystemCases,multiEquipmentCases,
  ...
  ...(['v3','v4','v5','v6','v7','v8'].includes(process.argv[3])?{unresolvedRepresentation}:{}),
  ...(['v4','v5','v6','v7','v8'].includes(process.argv[3])?{specificationDivergencePairs:specificationDifferences.size,specificationDifferences:[...specificationDifferences.values()]}:{}),
  ...(['v5','v6','v7','v8'].includes(process.argv[3])?{supportedMarkets:VEHICLE_DESTINATION_MARKETS,codeHashes:Object.fromEntries(await Promise.all(['src/lib/vehicle-market.ts','src/lib/mann-technical-applicability.ts','src/lib/mann-unified-technical-profile.ts'].map(async f=>[f,sha(await readFile(resolve(root,f),'utf8'))])))}:{}),
...
assert.ok(!process.argv[3]||['v2','v3','v4','v5','v6','v7','v8'].includes(process.argv[3]));
