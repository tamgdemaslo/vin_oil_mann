...
const [raw,sql,archive]=await Promise.all([readFile(resolve(dir,'nissan-xtrail-cvt-preview-v1.json'),'utf8'),readFile('/tmp/vehicle_fluid_requirements.sql','utf8'),readFile(resolve(root,'../vin-oil-mann/outputs/podbormasla-20260723/podbormasla_rows.ndjson'),'utf8')]);
...
for(const revision of s.revisions){
 ...
 const selected=engines.find(e=>!e.hybrid);assert.deepEqual(selected,revision.provenanceJson.sourceEngineBranch);
 ...
 for(const e of engines)for(const b of parsed.branches)for(let n=e.yearFrom*12;n<(e.yearTo+1)*12;n++)domain.add(key(e,b.condition.value,n));
 for(const b of revision.technicalDataJson.capacityBranches){
  assert.deepEqual(b.applicabilityJson.matchedEngineScope,[selected.engineCode]);assert.ok(parsed.branches.some(p=>p.sourceSegment===b.sourceSegment&&sha(p.condition)===sha(b.condition)));
  const w=b.applicabilityJson.window.intersection;for(let n=index(w.from);n<=index(w.to);n++){const k=key(selected,b.condition.value,n);assert.ok(domain.has(k));assert.ok(!assigned.has(k));assigned.add(k);covered++;}
 }
 for(const p of s.pending){
  ...
  for(let n=index(p.window.from);n<=index(p.window.to);n++){const k=key(p.sourceEngineBranch,p.condition.value,n);assert.ok(domain.has(k));assert.ok(!assigned.has(k));assigned.add(k);pending++;}
 }
 assert.deepEqual([...assigned].sort(),[...domain].sort());total+=domain.size;
}
assert.equal(total,432);assert.equal(covered,200);assert.equal(pending,232);assert.equal(s.pending.length,4);
...
await writeFile(resolve(dir,'nissan-xtrail-cvt-partition-v1.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(report));
