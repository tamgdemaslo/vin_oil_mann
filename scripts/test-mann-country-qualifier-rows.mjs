...
for(const label of ['2.0Xdi','1.616V','2.0 Country of vehicle manufacture Hungary','2.0 Syncro/4motion'])assert.equal(excluded(label),false,label);
...
const affected=rows.filter(r=>/^(?:FAHRZEUGPRODUKTIONSLAND|COUNTRYOFVEHICLEMANUFACTURE|SERIESERIES63)/.test(normalize(r.effectiveVehicleText??r.vehicleText).replace(/\s+/g,'')));
...
const referencedDrafts=plan.newRevisions.filter(r=>keys.has(r.vehicleVariantKey));assert.equal(referencedDrafts.length,0,'Current drafts must be reviewed before changing their retrieval path');
...
await writeFile(resolve(out,'mann-country-qualifier-audit-v1.json'),JSON.stringify({kind:'PDF_QUALIFIER_NOT_VEHICLE_AUDIT',inputHashes:{mann:sha(sql),plan:sha(planRaw),preflight:sha(preRaw),catalogCode:sha(await readFile(resolve(root,'src/lib/mann-catalog.ts'),'utf8'))},summary,
