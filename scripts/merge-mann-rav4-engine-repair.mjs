...
const reportPath=resolve(dir,'rav4-engine-merge-operations-v1.json'),mode=process.argv[2],raw=await readFile(path,'utf8');
if(mode==='prepare'){
 const batchRaw=await readFile(resolve(dir,'rav4-engine-scope-repair-drafts-v1.json'),'utf8'),batch=JSON.parse(batchRaw);
 const jointRaw=await readFile(resolve(dir,'rav4-engine-scope-joint-v1.json'),'utf8'),joint=JSON.parse(jointRaw),plan=JSON.parse(raw);
 assert.equal(sha(raw),'f0ea2b6ece3d08ce654b5d3e3b8d536ca0b419e7f996ee82f2997421f7ac4339');assert.equal(batch.planHash,plan.mercedesGainedScopedHistory.parentPlanHash);assert.equal(joint.planHash,sha(raw));assert.equal(joint.draftHash,sha(batchRaw));
 ...
 for(const d of batch.drafts){
  ...
  restored.applicabilityJson.matchedEngineScope=d.before.applicabilityJson.matchedEngineScope;delete restored.provenanceJson.engineIntersectionRepair;
 ...
 assert.equal(refs,0);
 const history={parentPlanHash:sha(raw),draftHash:sha(batchRaw),jointVerificationHash:sha(jointRaw),replacements:batch.drafts.map(d=>({originalRevisionId:d.before.id,successorId:d.after.id,removedCodes:d.after.provenanceJson.engineIntersectionRepair.removedCodes})),updatedActionReferences:refs,productionApplyAllowed:false};
 const anchor='  "kind": "PASSAT_INCLUSIVE_OFFLINE_PREVIEW",';
 operations.push({from:anchor,to:anchor+'\n'+JSON.stringify({rav4EngineScopeRepair:history},null,2).split('\n').slice(1,-1).join('\n')+','});
 let next=raw;for(const o of operations){assert.equal(next.split(o.from).length,2);next=next.replace(o.from,o.to);}
 const updated=JSON.parse(next);assert.equal(updated.newRevisions.length,1958);assert.equal(new Set(updated.newRevisions.map(r=>r.id)).size,1958);
 ...
 await writeFile(reportPath,JSON.stringify({parentPlanHash:sha(raw),afterPlanHash:sha(next),history,operations,otherRevisionsPreserved:1954},null,2)+'\n',{flag:'wx'});
 console.log(JSON.stringify({afterPlanHash:sha(next),revisions:4,actionReferences:refs,otherRevisionsPreserved:1954}));
}else{
 ...
 if(mode==='patch'){
  ...
  assert.equal(sha(restored),report.parentPlanHash);console.log(JSON.stringify({planHash:sha(raw),fullFileReversalVerified:true,otherRevisionsPreserved:1954,actionReferences:0}));
