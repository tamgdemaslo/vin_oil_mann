...
const progressGroups={basic:{status:'done',message:'saved'},transmission:{status:'searching',message:'searching'},drivetrain:{status:'queued',message:'queued'},auxiliary:{status:'failed',message:'Timeout'}};
const progressive=render(FluidResearchResults,{result:{...research('searching',[item]),message:'Завершено групп: 1 из 4',groups:progressGroups}});
assert.match(progressive,/Spec A/);assert.match(progressive,/Идёт поиск/);assert.match(progressive,/Ожидает поиска/);assert.match(progressive,/Поиск не завершён/);assert.doesNotMatch(progressive,/__skeleton/);
const retryPartial=render(FluidResearchResults,{result:{...research('saved',[item]),groups:{...progressGroups,transmission:{status:'done'},drivetrain:{status:'done'}}},onRetry(){}});
assert.match(retryPartial,/Повторить незавершённые/);assert.match(retryPartial,/Spec A/);
