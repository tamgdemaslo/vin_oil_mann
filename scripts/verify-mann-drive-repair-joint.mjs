...
const planRaw=await readFile(resolve(dir,'plan.json'),'utf8'),plan=JSON.parse(planRaw),repairRaw=await readFile(resolve(dir,'literal-drive-repair-v1.json'),'utf8'),repair=JSON.parse(repairRaw);assert.equal(sha(planRaw),repair.afterPlanHash);
...
for(const c of repair.changes){
 ...
 const group=all.filter(x=>x.vehicleVariantKey===r.vehicleVariantKey),n=x=>Number(x.slice(0,4))*12+Number(x.slice(5))-1,from=n(a.window.intersection.from),to=n(a.window.intersection.to);
 for(let m=from;m<=to;m++)for(const confirmedDrive of [undefined,'2WD','4WD']){
  ...
  const shouldHide=c.action==='HOLD_POWER_CONFLICT'||confirmedDrive!=='4WD';
...
const report={kind:'DRIVE_REPAIR_JOINT_LEGACY_CHECK',planHash:sha(planRaw),repairHash:sha(repairRaw),liveHash:sha(liveRaw),checks,positiveChecks,leakContexts:findings.length,findings,productionApplyAllowed:false,limitations:['Synthetic full-month source contexts; archived legacy with307planned supersessions simulated, not a production DB.','Checks repaired source identities, not every unrelated fluid or all VINs.']};
await writeFile(resolve(dir,'literal-drive-joint-v1.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify({...report,findings:findings.slice(0,2)}));
