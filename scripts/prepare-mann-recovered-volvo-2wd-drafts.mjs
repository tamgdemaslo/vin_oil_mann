...
const {parseConditionalFluidCapacities:parseConditional}=await j.import('../src/lib/fluid-capacity-conditions.ts');
const rawRows=new Map((await readFile(resolve(root,'../vin-oil-mann/outputs/podbormasla-20260723/podbormasla_rows.ndjson'),'utf8')).trim().split('\n').map(JSON.parse).map(r=>[r.row_id,r]));
const drafts=[];let checks=0;
for(const f of pre.findings.filter(f=>f.branch?.driveCondition==='2WD')){
 assert.ok(f.reasons.every(r=>['SOURCE_DRIVE_REQUIRES_EQUIPMENT_SCOPE','CAPACITY_OR_SERVICE_CONDITION','OWN_FLUID_APPLICATION_REQUIRES_REVIEW'].includes(r)));const s=sources.get(f.sourceRequirementId);assert.equal(sha(s),f.sourceHash);assert.equal(plan.newRevisions.some(r=>r.sourceRequirementId===s.id),false);
 ...
 const rawRow=rawRows.get(s.sourceRowId);assert.equal(sha(rawRow),f.rawRowHash);
 if(f.reasons.includes('OWN_FLUID_APPLICATION_REQUIRES_REVIEW')){assert.equal(s.systemCode,'FUEL_TANK');assert.equal(rawRow.application.trim(),'ОБЪЁМ БЕНЗОБАКА');assert.ok(!rawRow.model?.trim());}
 f.scope={...f.scope,requiredVehicleDrive:'2WD'};
 const conditional=f.reasons.includes('CAPACITY_OR_SERVICE_CONDITION');
 const policy=conditional?'MANN_CONDITIONAL_CAPACITY_PREVIEW_V1':'MANN_ENGINE_DATE_SCOPED_PREVIEW_V1';
 const parsed=conditional?parseConditional(s.fillVolumeText,s.systemCode):null;
 if(conditional){assert.equal(parsed.status,'structured');assert.deepEqual(parsed.branches.map(b=>b.condition),[{kind:'transmission',value:'manual'},{kind:'transmission',value:'automatic'}]);}
 ...
 if(conditional){delete technical.capacities;technical.capacityBranches=parsed.branches.map(b=>({condition:b.condition,sourceSegment:b.sourceSegment,applicabilityJson:f.scope,validation:f.target}));}
 ...
 for(let m=from-1;m<=to+1;m++)for(const engineCode of [f.scope.matchedEngineScope[0],'WRONG',undefined])for(const confirmedMarket of [f.scope.requiredMarket,'UNKNOWN',undefined])for(const confirmedDrive of [undefined,'2WD','4WD'])for(const transmission of [undefined,'manual','automatic']){
  const context={...f.scope.sourceVehicleScope,engineCode,confirmedMarket,confirmedDrive,productionMonth:`${Math.floor(m/12)}-${String(m%12+1).padStart(2,'0')}`},expected=confirmedDrive==='2WD'&&m>=from&&m<=to&&engineCode===f.scope.matchedEngineScope[0]&&(!f.scope.requiredMarket||confirmedMarket===f.scope.requiredMarket);
  const isolated=profile([fixture(r)],transmission,context),before=profile(existing,transmission,context),after=profile([...existing,fixture(r)],transmission,context);assert.equal(isolated.items.length,expected?1:0);
  if(expected){const item=after.items.find(i=>i.revisionId===r.id);assert.ok(item);assert.deepEqual(item.capacities,isolated.items[0].capacities);if(conditional)assert.deepEqual(item.capacities.map(c=>c.nominalLiters),transmission?[transmission==='manual'?9.5:10]:[]);assert.deepEqual(item.specifications,isolated.items[0].specifications);
