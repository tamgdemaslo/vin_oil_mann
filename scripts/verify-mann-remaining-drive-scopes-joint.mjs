...
const planRaw=await readFile(resolve(dir,'plan.json'),'utf8'),plan=JSON.parse(planRaw),repairRaw=await readFile(resolve(dir,'remaining-drive-scope-repair-v1.json'),'utf8'),repair=JSON.parse(repairRaw);assert.equal(sha(planRaw),repair.afterPlanHash);
...
for(const c of repair.changes){
 ...
 for(let m=from;m<=to;m++)for(const confirmedDrive of [undefined,'2WD','4WD'])for(const engineCode of [...c.before.applicabilityJson.matchedEngineScope,'WRONG',undefined]){
  ...
  const shouldHide=!a.matchedEngineScope.includes(engineCode)||(a.requiredVehicleDrive&&confirmedDrive!==a.requiredVehicleDrive)||context.productionMonth<a.window.intersection.from||context.productionMonth>a.window.intersection.to;
...
const report={kind:'REMAINING_DRIVE_SCOPE_JOINT_LEGACY_CHECK',planHash:sha(planRaw),repairHash:sha(repairRaw),liveHash:sha(liveRaw),checks,positiveChecks,leakContexts:findings.length,findings,productionApplyAllowed:false,limitations:['Synthetic full-month source contexts; archived legacy with307planned supersessions simulated, not a production DB.','Checks repaired source identities, not every unrelated fluid or all VINs.']};
await writeFile(resolve(dir,'remaining-drive-scope-joint-v1.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify({...report,findings:findings.slice(0,2)}));
