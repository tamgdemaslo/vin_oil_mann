...
const planRaw=await readFile(resolve(root,'outputs/mann-gentra-evidence-review-2026-09-14/plan.json'),'utf8'),plan=JSON.parse(planRaw);assert.equal(sha(planRaw),'8429d8f48e0c59d049ca04f69b2ee8156ceee4aff03b7b21ef94b6395af9e778');
...
const findings=[];
for(const r of plan.newRevisions.filter(r=>r.applicabilityJson.driveType!=null)){
 ...
 const evidence=anchors.map(row=>{
  let parsed=parse(row.application,s.makeNormalized);
 ...
 findings.push({revisionId:r.id,revisionHash:sha(r),sourceRequirementId:s.id,sourceHash:sha(s),vehicleVariantKey:r.vehicleVariantKey,systemCode:r.systemCode,legacyDriveType:a.driveType,scope:a,evidence,branches,coveringBranches:covering,classification,literalDrives:drives,observations,missingLiteralDriveGate:classification==='LITERAL_SINGLE_DRIVE'&&observations.some(o=>o.confirmedDrive!==drives[0]&&o.visible),productionApplyAllowed:false});
}
assert.equal(findings.length,44);
const summary={revisions:44,sourceRecords:new Set(findings.map(f=>f.sourceRequirementId)).size,classifications:Object.fromEntries([...Map.groupBy(findings,f=>f.classification)].map(([k,a])=>[k,a.length])),missingLiteralDriveGate:findings.filter(f=>f.missingLiteralDriveGate).length};
await writeFile(resolve(dir,'legacy-vehicle-drive-audit-v1.json'),JSON.stringify({kind:'LEGACY_DRIVE_SOURCE_AUDIT',planHash:sha(planRaw),sourceHash:sha(sql),rawHash:sha(raw),summary,findings,productionApplyAllowed:false,limitations:['Own raw engine tables and same-table anchors inspected; no inferred drive from legacy awd field.','Single-drive result is a defect candidate, not an automatic scope rewrite: exact power/branch/years still require validation.','Synthetic single-record display probes, not a real VIN or live database.']},null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(summary));
