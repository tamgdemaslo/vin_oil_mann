...
const [sql,mannRaw,planRaw,auditRaw,rawText]=await Promise.all([readFile('/tmp/vehicle_fluid_requirements.sql','utf8'),readFile('/tmp/mann_filter_applications.sql','utf8'),readFile(resolve(dir,'plan.json'),'utf8'),readFile(resolve(root,'outputs/mann-corolla-cvt-preview-2026-09-14/remaining-cvt-capacity-recheck-v1.json'),'utf8'),readFile(resolve(root,'../vin-oil-mann/outputs/podbormasla-20260723/podbormasla_rows.ndjson'),'utf8')]);
const plan=JSON.parse(planRaw),audit=JSON.parse(auditRaw);assert.equal(audit.sourceHash,sha(sql));assert.equal(audit.mannHash,sha(mannRaw));const auditParentRaw=await readFile(resolve(root,'outputs/mann-corolla-cvt-preview-2026-09-14/plan.json'),'utf8');assert.equal(audit.planHash,sha(auditParentRaw));
...
const forms=new Set(mannMakeFormsForTest('NISSAN')),rows=parseCopy(mannRaw,'mann_filter_applications').filter(r=>forms.has(normalizeMannText(r.makeNormalized||r.make))),variants=Map.groupBy(rows,r=>r.vehicleVariantKey);
...
const ids=['1d19bedfa9fd5c3f43b4148dee739f08ed54a7ec4e9cd24d327f0f6b10cfa8fa'];
...
for(const id of ids){
 ...
 const engines=nissanXtrailEngineBranches(anchors[0].model);assert.equal(engines?.length,2);const engine=engines.find(e=>!e.hybrid);assert.equal(engine.powerHp,source.powerHp);assert.equal(engine.yearFrom,source.yearFrom);assert.equal(engine.yearTo,source.yearTo);
 ...
 for(const held of engines.filter(e=>e.hybrid))for(const b of parsed.branches){
  total+=(held.yearTo-held.yearFrom+1)*12;
  pending.push({sourceRequirementId:id,sourceHash:sha(original),originalSource:original,sourceEngineBranch:held,condition:b.condition,window:{from:`${held.yearFrom}-01`,to:`${held.yearTo}-12`},reason:'HYBRID_ENGINE_NOT_MATCHED',publicationAllowed:false});
 }
 ...
 for(let n=source.yearFrom*12-1;n<=(source.yearTo+1)*12;n++)for(const engineCode of [engine.engineCode,...engines.filter(e=>e.hybrid).map(e=>e.engineCode),'WRONG',undefined])for(const type of [undefined,'manual','cvt','automatic','robot']){
