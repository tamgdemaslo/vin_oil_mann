...
import {nissanEnginePowerBranches} from './lib/mann-nissan-engine-power-branches.mjs';
...
const root=resolve(import.meta.dirname,'..'),dir=resolve(root,process.argv[2]??'outputs/mann-nissan-xtrail-cvt-list-preview-2026-09-14');
const [sql,mannRaw,planRaw,inventoryRaw,archive]=await Promise.all([readFile('/tmp/vehicle_fluid_requirements.sql','utf8'),readFile('/tmp/mann_filter_applications.sql','utf8'),readFile(resolve(dir,'plan.json'),'utf8'),readFile(resolve(root,'outputs/mann-nissan-xtrail-manual-preview-2026-09-14/cvt-model-list-impact-v1.json'),'utf8'),readFile(resolve(root,'../vin-oil-mann/outputs/podbormasla-20260723/podbormasla_rows.ndjson'),'utf8')]);
const plan=JSON.parse(planRaw),inventory=JSON.parse(inventoryRaw);assert.equal(inventory.sourceHash,sha(sql));assert.equal(inventory.planHash,sha(await readFile(resolve(root,'outputs/mann-nissan-xtrail-manual-preview-2026-09-14/plan.json'),'utf8')));
...
for(const item of inventory.parsed.filter(i=>!['92d27ff8293e1f1b5cc64f99948bd0a6314587586e041126cb2e0811f8fa76fd','5e51c7ba7508e7c62f06e9d5d2f7a3bcdfcf901de8e4d78c304940b887752350'].includes(i.sourceRequirementId))){
 const base=sources.get(item.sourceRequirementId),original=overlay.originalById.get(base.id);assert.equal(sha(original),item.sourceHash);assert.equal(base.make,'nissan');assert.deepEqual(models(original.componentModel),item.models);
 ...
 assert.equal(anchors.length,1);const engineBranches=nissanEnginePowerBranches(anchors[0].model,anchors[0].production_years);assert.ok(engineBranches);for(const engineBranch of engineBranches){
 assert.equal(engineBranch.yearFrom,base.yearFrom);assert.equal(engineBranch.yearTo,base.yearTo);
 const source={...base,engineCodeNormalized:engineBranch.engineCode,engineCodesJson:[engineBranch.engineCode],powerHp:engineBranch.powerHp,yearFrom:engineBranch.yearFrom,yearTo:engineBranch.yearTo};
 ...
 results.push({sourceEngineBranch:engineBranch,sourceRequirementId:source.id,sourceHash:sha(original),originalSource:original,models:item.models,metadata,capacity,rawEngineAnchors:anchors,initial,attempts,publicationAllowed:false});
}
}
assert.equal(results.length,20);
const candidates=results.flatMap(r=>r.attempts.filter(a=>!a.reasons.length).map(a=>({sourceEngineBranch:r.sourceEngineBranch,sourceRequirementId:r.sourceRequirementId,models:r.models,...a})));
const files=['scripts/recheck-mann-nissan-cvt-power-branches.mjs','scripts/lib/mann-nissan-engine-power-branches.mjs','scripts/lib/mann-conditional-vehicle-identity.mjs','scripts/lib/mann-gearbox-source-quality.mjs','src/lib/mann-fluid-matcher-v2.ts','src/lib/mann-vehicle-resolver.ts','src/lib/mann-engine-code-list.ts','src/lib/vehicle-normalization.ts','src/lib/fluid-capacity-parser.ts','src/lib/fluid-source-system-context.ts','src/lib/mann-transmission-component.ts','src/lib/mann-transmission-model-list.ts','src/lib/mann-unified-technical-profile.ts','src/lib/mann-technical-applicability.ts'];
const report={sourceHash:sha(sql),mannHash:sha(mannRaw),planHash:sha(planRaw),inventoryHash:sha(inventoryRaw),rawHash:sha(archive),codeHashes:Object.fromEntries(await Promise.all(files.map(async f=>[f,sha(await readFile(resolve(root,f),'utf8'))]))),summary:{sources:new Set(results.map(r=>r.sourceRequirementId)).size,sourceEnginePowerBranches:results.length,attempts:results.reduce((n,r)=>n+r.attempts.length,0),candidatePairs:candidates.length,candidateSources:new Set(candidates.map(c=>c.sourceRequirementId)).size},candidates,results,productionApplyAllowed:false,limitation:'Raw engine/power alternatives recovered explicitly; still conditional candidates requiring runtime, source partition and predecessor checks. Top-three retrieval, not exhaustive matching or confirmed installed gearboxes.'};
await writeFile(resolve(dir,'nissan-cvt-power-branch-recheck-v1.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(report.summary));
