...
const path=resolve(dir,'plan.json'),reportPath=resolve(dir,'remaining-scoped-merge-v1.json'),mode=process.argv[2];
const raw=await readFile(path,'utf8');
if(mode==='prepare'){
 const plan=JSON.parse(raw), inputs=[];
 for(const name of ['remaining-unplanned-drafts-v3.json','unplanned-conditional-capacity-drafts-v1.json','unplanned-compound-capacity-drafts-v1.json','rav4-unambiguous-year-capacity-draft-v1.json']){
 ...
 assert.deepEqual(inputs.map(i=>i.data.newRevisions.length),[11,2,1,1]);
 assert.deepEqual(inputs.map(i=>i.data.summary.profileChecks??i.data.summary.checks),[6408,6120,405,972]);
 const draft={newRevisions:inputs.flatMap(i=>i.data.newRevisions)},draftRaw=JSON.stringify(inputs.map(({name,hash})=>({name,hash})));
 assert.equal(plan.newRevisions.length,1992);assert.equal(new Set(draft.newRevisions.map(r=>r.id)).size,15);
 ...
 for(const r of draft.newRevisions){
  ...
  const policy=r.provenanceJson.catalogPreviewPolicy; const fingerprint=policy==='MANN_CONDITIONAL_CAPACITY_PREVIEW_V1'?sha({key:`${r.sourceRequirementId}:${r.vehicleVariantKey}`,policy,applicabilityJson:r.applicabilityJson,technicalDataJson:r.technicalDataJson}):sha({policy,sourceRequirementId:r.sourceRequirementId,vehicleVariantKey:r.vehicleVariantKey,applicability:r.applicabilityJson,technical:r.technicalDataJson});
 ...
 const summary={...plan.summary,candidateRevisions:all.length,sourceRequirements:new Set(all.map(r=>r.sourceRequirementId)).size,remainingScopedRevisions:additions.length};
 ...
 const updated=JSON.parse(next);assert.equal(updated.newRevisions.length,2007);assert.equal(new Set(updated.newRevisions.map(r=>r.id)).size,2007);
 assert.deepEqual(updated.newRevisions.slice(15),plan.newRevisions);assert.deepEqual(updated.existingActions,plan.existingActions);assert.deepEqual(updated.protectedBranches,plan.protectedBranches);
 await writeFile(reportPath,JSON.stringify({parentPlanHash:sha(raw),afterPlanHash:sha(next),groups,summary,added:15,preserved:1992,actionsChanged:0,publicationAllowed:false},null,2)+'\n',{flag:'wx'});
 ...
 if(mode==='patch'){
  ...
  assert.equal(sha(restored),report.parentPlanHash);console.log(JSON.stringify({wholeFileReversalVerified:true,planHash:sha(raw),added:15,preserved:1992,actionsChanged:0}));
