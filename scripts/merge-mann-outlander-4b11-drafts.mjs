...
const root=resolve(import.meta.dirname,'..'),dir=resolve(root,'outputs/mann-gentra-evidence-review-2026-09-14'),path=resolve(dir,'plan.json'),out=resolve(dir,'outlander-4b11-merge-v1.json');
const raw=await readFile(path,'utf8'),mode=process.argv[2];
if(mode==='prepare'){
 const plan=JSON.parse(raw),draftRaw=await readFile(resolve(dir,'outlander-4b11-fluid-drafts-v1.json'),'utf8'),draft=JSON.parse(draftRaw);assert.equal(sha(raw),draft.planHash);assert.equal(draft.productionApplyAllowed,false);assert.deepEqual(draft.summary,{drafts:3,replacements:0,profileChecks:18432});
 const matchRaw=await readFile(resolve(dir,'outlander-4b11-source-rematch-v1.json'),'utf8'),match=JSON.parse(matchRaw);assert.equal(sha(matchRaw),draft.rematchHash);for(const [file,hash] of Object.entries(match.runtimeHashes))assert.equal(sha(await readFile(resolve(root,file),'utf8')),hash);
 ...
 const summary={...plan.summary,candidateRevisions:all.length,sourceRequirements:new Set(all.map(r=>r.sourceRequirementId)).size,actions:{},outlander4B11Revisions:3};for(const a of nextActions)summary.actions[a.action]=(summary.actions[a.action]??0)+1;
 ...
 const updated=JSON.parse(next);assert.deepEqual(updated.newRevisions.slice(3),plan.newRevisions);assert.deepEqual(updated.existingActions,nextActions);assert.deepEqual(updated.protectedBranches,plan.protectedBranches);assert.equal(new Set(updated.newRevisions.map(r=>r.id)).size,2046);
 await writeFile(out,JSON.stringify({parentPlanHash:sha(raw),afterPlanHash:sha(next),draftHash:sha(draftRaw),groups,summary,publicationAllowed:false},null,2)+'\n',{flag:'wx'});console.log(JSON.stringify({afterPlanHash:sha(next),revisions:2046,actionsChanged:0,groups:groups.length}));
}else{
