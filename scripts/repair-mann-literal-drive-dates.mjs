...
const root=resolve(import.meta.dirname,'..'),dir=resolve(root,'outputs/mann-gentra-evidence-review-2026-09-14'),path=resolve(dir,'plan.json'),reportPath=resolve(dir,'literal-drive-date-repair-v1.json');
const raw=await readFile(path,'utf8'),mode=process.argv[2];
if(mode==='prepare'){
 const plan=JSON.parse(raw),auditRaw=await readFile(resolve(root,'outputs/mann-current-full-rematch-2026-09-14-v2/legacy-vehicle-drive-audit-v1.json'),'utf8'),audit=JSON.parse(auditRaw);assert.equal(plan.literalVehicleDriveRepair.parentPlanHash,audit.planHash);assert.equal(sha(raw),'73f45c53e8195ad4c95bca2ca58fbde3c097b2538900e28ce8a3da0f637c31df');
 ...
 for(const f of audit.findings.filter(f=>f.classification==='BRANCH_DATES_NARROWER_THAN_DRAFT')){
  ...
  assert.equal(f.branches.length,1);const branch=f.branches[0];assert.ok(['4WD','2WD, 4WD'].includes(branch.driveCondition));
  ...
  if(powerMatches){
   const narrow=scope=>{
    ...
    if(branch.driveCondition==='4WD')scope.requiredVehicleDrive='4WD';
   };
   narrow(after.applicabilityJson);
   for(const b of after.technicalDataJson.capacityBranches??[])narrow(b.applicabilityJson);
   after.provenanceJson.literalVehicleDriveDateRepair={auditHash:sha(auditRaw),originalRevisionId:before.id,originalRevisionHash:sha(before),branch,mannPowerHp:powers[0],publicationAllowed:false};
  ...
  for(let m=from-1;m<=to+1;m++)for(const confirmedDrive of [undefined,'2WD','4WD']){
   ...
   const narrowed=after.applicabilityJson.window.intersection;const expected=(!after.applicabilityJson.requiredVehicleDrive||confirmedDrive==='4WD')&&context.productionMonth>=narrowed.from&&context.productionMonth<=narrowed.to;assert.equal(next.items.length,expected?old.items.length:0);
 ...
 assert.equal(changes.length,12);assert.equal(ids.size,12);
 ...
 operations.push({from:anchor,to:anchor+'\n'+JSON.stringify({literalVehicleDriveDateRepair:history},null,2).split('\n').slice(1,-1).join('\n')+','});
