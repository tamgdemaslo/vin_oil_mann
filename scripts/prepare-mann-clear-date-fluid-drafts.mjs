...
const preRaw=await readFile(resolve(dir,'confirmed-date-fluid-preflight-v1.json'),'utf8'),pre=JSON.parse(preRaw),planRaw=await readFile(resolve(root,'outputs/mann-gentra-evidence-review-2026-09-14/plan.json'),'utf8'),plan=JSON.parse(planRaw);assert.equal(sha(planRaw),pre.planHash);
...
for(const f of pre.findings.filter(f=>f.status==='READY_FOR_SCOPED_DRAFT')){
 ...
 for(let m=from-1;m<=to+1;m++)for(const engineCode of [f.scope.matchedEngineScope[0],'WRONG',undefined])for(const confirmedMarket of [f.scope.requiredMarket,'UNKNOWN',undefined]){
  const context={...f.scope.sourceVehicleScope,engineCode,confirmedMarket,productionMonth:`${Math.floor(m/12)}-${String(m%12+1).padStart(2,'0')}`},expected=m>=from&&m<=to&&engineCode===f.scope.matchedEngineScope[0]&&confirmedMarket===f.scope.requiredMarket;
