...
const planRaw=await readFile(resolve(dir,'plan.json'),'utf8'),plan=JSON.parse(planRaw),matchRaw=await readFile(resolve(dir,'outlander-4b11-source-rematch-v1.json'),'utf8'),match=JSON.parse(matchRaw);assert.equal(sha(planRaw),match.planHash);
...
for(const r of drafts){const a=r.applicabilityJson,group=drafts.filter(d=>d.vehicleVariantKey===r.vehicleVariantKey),replaced=new Set(group.flatMap(d=>d.replacesRevisionIds));
 ...
 for(let m=month(a.window.intersection.from)-1;m<=month(a.window.intersection.to)+1;m++)for(const engineCode of [a.matchedEngineScope[0],'4J11','WRONG',undefined])for(const confirmedMarket of [a.requiredMarket,'JP',undefined])for(const transmission of ['manual','automatic','cvt',undefined]){
